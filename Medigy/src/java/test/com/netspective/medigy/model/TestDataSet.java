...
public class TestDataSet extends TestCase
{
    public void testDataSet()
    {
        ...
        session.setProcessName(TestDataSet.class.getName() + ".testDataSet()");
        HibernateUtil.getSession().save(session);
        SessionManager.getInstance().setActiveSession(session);

        Criteria criteria = HibernateUtil.getSession().createCriteria(Party.class);
        criteria.add(Expression.eq("partyName", "SYS_GLOBAL_PARTY"));
        Party globalParty = (Party) criteria.uniqueResult();
        assertNotNull(globalParty);

        criteria = HibernateUtil.getSession().createCriteria(PartyRoleType.class);
        criteria.add(Expression.eq("label", "Employee"));
        ...
        assertEquals(subsidiaryRoleType.getLabel(), "Employee");
