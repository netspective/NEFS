...
/**
 * $Id: DateComparison.java,v 1.1 2003-03-13 18:25:44 shahid.shah Exp $
 */
...
public class DateComparison extends BinaryOpComparison
{
    ...
    {
        statement.addParam(new ConcatenateValueSource(null, cond.getValue(), null));
        String retString = "";
        String bindExpression = cond.getBindExpr();
        if(bindExpression != null && bindExpression.length() > 0)
            retString = " TO_DATE(" + cond.getField().getWhereExpr() + " , 'MM/DD/YYYY')" + sqlExpr + " TO_DATE(" + bindExpression + ",'MM/DD/YYYY')";
        else
